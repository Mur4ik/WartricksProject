...
public class FramedMenu {
    ...
    public void addButton(String label, ChangeListener listener, boolean active) {
        this.addButton(label, "", listener, active);
    }

    // Adds a button to the menu, with a secondary label (like MP cost) aligned to the right
    public void addButton(String label, String secondaryLabel, ChangeListener listener,
            boolean active) {
        LabelStyle style;
        if (active) {
        ...
        final Label l = new Label(label, style);
        final Button b = new Button(skin.get(ButtonStyle.class));
